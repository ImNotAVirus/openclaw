...
- **Per-host allowlists** (`exec-approvals.json` map format): each agent can have separate allowlist entries per exec host (`"gateway"`, `"sandbox"`, `"node"`) plus a `"default"` fallback. This prevents a binary allowed on sandbox from implicitly being allowed on gateway. When `tools.exec.allowedHosts` is set, agents may request a more isolated host than the configured default at runtime — always more isolated, never less.
